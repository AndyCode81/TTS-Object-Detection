...
def attempt_load(weights, map_location=None):
    # Remove any unsafe globals addition
    model = Ensemble()

    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
        ckpt = torch.load(w, map_location=map_location)  # load the checkpoint

        # Load the model (ema or model weights)
        model.append(ckpt['ema' if ckpt.get('ema') else 'model'].float().fuse().eval())  # FP32 model
    
    # Compatibility updates for older versions of PyTorch
    ...
    if len(model) == 1:
        return model[-1]  # return the model
    else:
